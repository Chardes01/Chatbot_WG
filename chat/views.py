...
class Index(LoginRequiredMixin, View):
    ...
    def post(self, request, chatid):
        ...
        #####veränderung!!!!!!######
        content,end = botobject.chat(content, chatsession)
        
        if not end:
            #time.sleep(3)
            ChatMessage.objects.create(user_message=False, session=chatsession, content=content)
            chatsession.save() 
        else: 
            chatsession.active = False #test
            chatsession.save() 



        ####veränderung vorbei, vorheriges:
